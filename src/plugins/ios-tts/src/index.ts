/**
 * iOS TTS Plugin — Capacitor 6 web shim
 *
 * 在 iOS 上：registerPlugin 找到 IosTtsPlugin.swift，走 AVSpeechSynthesizer（系统自带，完全离线）
 * 在 Android 上：回退到已有的 NativeTts 插件
 * 在浏览器上：fallback 用 speechSynthesis
 */
import { registerPlugin } from '@capacitor/core';
import type { NativeTtsOptions } from '../../native-tts/src/definitions';
import type { NativeTtsPlugin } from '../../native-tts/src/definitions';

// 原生插件（iOS 端）
const _raw = registerPlugin<NativeTtsPlugin>('IosTts');
...
export const IosTts: NativeTtsPlugin = _plugin;
